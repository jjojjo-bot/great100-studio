...
function V21CaptionEditor({ scene, onChange }: { scene: Scene; onChange: (scene: Scene) => void }) {
  ...
  return <div className="panel v21-caption-panel">
    <label><strong>강조 자막 · subtitle</strong><small>내레이션 전체 자막과 별개로 장면 시작에 잠깐 크게 표시됩니다.</small><textarea aria-label="강조 자막" value={scene.subtitle || ""} onChange={(event) => onChange({ ...scene, subtitle: event.target.value, title: event.target.value })} /></label>
