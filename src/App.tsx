...
function V21CaptionEditor({ project, scene, onChange }: { project: ProjectData; scene: Scene; onChange: (scene: Scene) => void }) {
  const originalCaptions = project.source?.scenes.find((item) => item.id === (scene.source_scene_id || scene.id))?.captions;
  ...
  return <div className="panel v21-caption-panel">
    ...
    {!scene.narration_audio && <CaptionWaveform scene={scene} originalCaptions={originalCaptions} onChange={onChange} />}
