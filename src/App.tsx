...
import { createProjectOnDisk, deleteCandidateAsset, deleteProject, downloadBlob, exportProjectZip, generateImages, getAccessCode, getBackgroundMusic, getRenderedVideo, importImageCandidates, importSceneVideo, isTauri, listProjects, removeBackgroundMusic, removeSceneNarration, saveBackgroundMusic, saveProject, saveRenderedVideo, setAccessCode } from "./platform";
...
function SceneStudio({ project, onChange, onProjectChange }: { project: ProjectData; onChange: (scene: Scene) => void; onProjectChange: (project: ProjectData) => Promise<void> }) {
  ...
  return <div className="page scene-page">
    ...
    <nav className="scene-navigation" aria-label="장면 이동"><button className="btn ghost scene-nav-button" disabled={position === 0} onClick={() => switchScene(project.scenes[position - 1].id)}><ArrowLeft size={15} /> 이전 Scene</button><div className="scene-tabs">{project.scenes.map((item) => <button key={item.id} className={item.id === scene.id ? "active" : ""} aria-current={item.id === scene.id ? "step" : undefined} onClick={() => switchScene(item.id)}><span>{item.selected_candidate_id ? <Check size={13} /> : item.number}</span>{item.title}{item.narration_audio && <Mic size={12} aria-label="녹음 있음" />}</button>)}</div><button className="btn primary scene-nav-button" disabled={position === project.scenes.length - 1} onClick={() => switchScene(project.scenes[position + 1].id)}>다음 Scene <ArrowRight size={15} /></button></nav>
    ...
    <div className="scene-workspace"><div className="scene-workspace-controls">
    ...
    {project.schema_version !== 1 && neighbors.length > 0 && <details className="panel neighbor-panel"><summary>인접 장면 비교 <small>필요할 때 펼쳐보기</small></summary><div className="neighbor-grid">{neighbors.map((item) => { const image = item.candidates.find((candidate) => candidate.id === item.selected_candidate_id); return <div key={item.id}>{image ? <img src={image.preview_url} alt={`${item.title} 선택 이미지`} /> : <div className="neighbor-empty">아직 이미지 없음</div>}<small>{item.source_scene_id} · {item.title}</small></div>; })}</div></details>}
    ...
    </div><ScenePreviewPane key={scene.id} project={project} scene={scene} onChange={onChange} /></div>
  </div>;
}
